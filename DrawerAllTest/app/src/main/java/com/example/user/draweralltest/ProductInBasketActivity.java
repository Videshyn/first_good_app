...
public class ProductInBasketActivity extends Activity implements View.OnClickListener {
    ...
    private  void initUI(){
        ...
        btnAdd.setText("Delete from Basket");
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.btn_basket){
            Toast.makeText(ProductInBasketActivity.this, "Deleted", Toast.LENGTH_SHORT).show();
            try {
                dbHelper.createDataBase();
                dbHelper.openDataBase();
            } catch (SQLException e) {
            ...
            cv.put("p_basket", "false");
            int id = database.update("Products", cv, "p_name=?", new String[] {name.getText().toString()});
            database.close();
            dbHelper.close();
            Intent intent = new Intent(ProductInBasketActivity.this, MainActivity.class);
